//! SAM3 configuration — mirrors `mlx-gen-sam3`'s `config.rs` (and `transformers/models/sam3`) for
//! the `facebook/sam3` checkpoint. Pure data: hardcoded shipped defaults, no JSON parsing (the
//! reference parses the config JSON; this port mirrors the values, like the MLX twin).
//!
//! Types use `usize` for tensor dims (candle reshape/permute take `usize`) and `f64` for LayerNorm
//! epsilons (candle `LayerNorm::new` takes `f64`); the numeric values are identical to the MLX side.
...
impl Sam3VisionConfig {
    ...
    pub fn pretrain_grid(&self) -> usize {
        self.pretrain_image_size / self.patch_size
    }
}

/// DETR detector configuration (encoder + decoder + presence + scoring). The shared working width is
/// `hidden_size` (256); the vision/text features are already projected to it upstream.
#[derive(Clone, Debug)]
...
/// Geometry/exemplar prompt-encoder configuration — the box/point **PVS** prompt path (sc-6244).
/// Numerically the same working width as the DETR stack (256/8 heads/2048 FFN), with a distinct
/// `roi_size`.
#[derive(Clone, Debug)]
...
/// `text_projection` output dim. Defaults are the shipped `facebook/sam3` values (slice sc-6241).
#[derive(Clone, Debug)]
