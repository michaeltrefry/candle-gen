//! 5-point landmark alignment — the classical-CV glue between the SCRFD detector ([`crate::scrfd`])
//! and the ArcFace embedding ([`crate::iresnet`]). A faithful port of mlx-gen-face's `align.rs`:
//! every routine here is pure host math (`f64`/`u8`/`Vec`), identical to the MLX sibling, so the
//! Umeyama similarity solver and the fixed-point [`warp_affine`] are bit-for-bit the same on both
//! backends. The only candle-specific routine is [`to_arcface_input`], which packs the aligned crops
//! into the NCHW tensor candle's ArcFace forward consumes (MLX packs NHWC).
//!
//! [`norm_crop`] (112²) is **insightface-faithful** — `glintr100`/PuLID/InstantID expect *this exact*
//! alignment, so it reproduces `insightface.utils.face_align.norm_crop` numerically (Umeyama 2×3 `M`
...
/// facexlib FFHQ 512² template (`FaceRestoreHelper`, `face_size=512`, `crop_ratio=(1,1)`). Same
/// 5-point order as [`ARCFACE_DST`]. Reserved for the PuLID 512² crop (sc-5492).
...
impl Affine2x3 {
    /// cv2 `invertAffineTransform` (the inverse map `warpAffine` applies internally when the supplied
    /// `M` is *forward*, src→dst, without `WARP_INVERSE_MAP`).
...
/// insightface `estimate_norm` calls).
...
/// Pack 112² RGB `u8` crops into the ArcFace input batch: **NCHW** `[N,3,112,112]` f32, normalized
/// `(rgb - 127.5) / 127.5` (the antelopev2 ArcFace blob). MLX packs the same data NHWC; candle's
/// `conv2d` is NCHW, so the crops are built NHWC then permuted to NCHW here.
pub fn to_arcface_input(crops: &[Vec<u8>], device: &Device) -> Result<Tensor> {
    let n = crops.len();
    let mut data = Vec::with_capacity(n * 112 * 112 * 3);
    for crop in crops {
        debug_assert_eq!(crop.len(), 112 * 112 * 3, "crop must be 112×112×3");
        data.extend(crop.iter().map(|&v| (v as f32 - 127.5) / 127.5));
    }
    let nhwc = Tensor::from_vec(data, (n, 112, 112, 3), device)?;
...
mod tests {
    ...
    /// The similarity solver must agree with skimage on a known transform (isolated from f32 kps
    /// quantization). Map the template by a fixed scale·R + translation, recover the inverse.
    #[test]
    fn umeyama_recovers_a_known_similarity() {
        let (s, ct, st, tx, ty) = (1.7f64, 0.8f64, 0.6f64, 12.0f64, -5.0f64);
