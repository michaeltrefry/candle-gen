...
//! done on the host in f32; the tensor ops apply the scalars via `affine`. A direct port of
//! `mlx-gen-svd`'s `scheduler.rs`.
...
pub fn scale_model_input(x: &Tensor, sigma: f32) -> Result<Tensor> {
    let c_in = 1.0 / ((sigma as f64).powi(2) + 1.0).sqrt();
    x.affine(c_in, 0.0)
}

/// v-prediction → predicted clean sample (diffusers `step`):
/// `x̂0 = v·(−σ/sqrt(σ²+1)) + x/(σ²+1)`.
pub fn v_pred_denoised(model_output: &Tensor, sample: &Tensor, sigma: f32) -> Result<Tensor> {
    let s2 = (sigma as f64).powi(2);
    let c_out = -(sigma as f64) / (s2 + 1.0).sqrt();
    let c_skip = 1.0 / (s2 + 1.0);
    model_output.affine(c_out, 0.0)? + sample.affine(c_skip, 0.0)?
}
...
pub fn euler_step(
    ...
    // x' = sample + (sample - denoised) · (dt/σ).
    sample + (sample - denoised)?.affine(dt / sigma as f64, 0.0)?
}
...
mod tests {
    ...
    #[test]
    fn v_pred_and_euler_match_formulas() -> Result<()> {
        let dev = Device::Cpu;
        let v = Tensor::from_slice(&[0.5f32, -0.5], 2, &dev)?;
        let x = Tensor::from_slice(&[2.0f32, 4.0], 2, &dev)?;
        let sigma = 3.0f32;
        let den = v_pred_denoised(&v, &x, sigma)?;
        let s2 = sigma * sigma;
        let vv = v.to_vec1::<f32>()?;
        let xv = x.to_vec1::<f32>()?;
        let want: Vec<f32> = (0..2)
            .map(|i| vv[i] * (-sigma / (s2 + 1.0).sqrt()) + xv[i] / (s2 + 1.0))
            .collect();
        for (g, w) in den.to_vec1::<f32>()?.iter().zip(&want) {
            assert!((g - w).abs() < 1e-5, "v_pred {g} vs {w}");
        }
        // euler.
        let nxt = euler_step(&x, &den, sigma, 1.5)?;
        let dv = den.to_vec1::<f32>()?;
        let want_e: Vec<f32> = (0..2)
            .map(|i| xv[i] + (xv[i] - dv[i]) / sigma * (1.5 - sigma))
            .collect();
        for (g, w) in nxt.to_vec1::<f32>()?.iter().zip(&want_e) {
